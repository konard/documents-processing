...
// (e.g. "KONSTANTIN[REDACTED]"). Supply a JSON de-glue map with --glued=<file.json>
// — { "<GLUEDTOKEN>": "<TWO WORDS>" } — to split them back apart so filenames
// stay consistent. Without it, names are used verbatim.
//
// Usage:  node first-page-tickets.js <inputDir> [outputDir] [--glued=<file.json>]
//         node first-page-tickets.js <inputDir> [outputDir] --pages=1
//
...
if (!INPUT_DIR) {
  console.error(
    'Usage: node first-page-tickets.js <inputDir> [outputDir] [--glued=<file.json>] [--pages=<n>]'
  );
  process.exit(1);
}
const OUT_DIR =
