...
//
// The letter is (re)rendered from Markdown via markdown-to-pdf.js, so editing
...
//
// Usage:
//   node join-letter-attachments.js <letter.md> <output.pdf> <attachment1.pdf> [attachment2.pdf ...]
//
...
if (!letterMd || !outputPdf || attachments.length === 0) {
  console.error(
    'Usage: node join-letter-attachments.js <letter.md> <output.pdf> <attachment1.pdf> [attachment2.pdf ...]'
  );
  process.exit(1);
}
...
execFileSync(
  'node',
  [
    path.join(__dirname, 'markdown-to-pdf.js'),
    letterMd,
    letterPdf,
    '--fit-one-page',
  ],
