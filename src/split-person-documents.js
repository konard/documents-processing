...
//
// Recompression is delegated to compress-pdf.js (resolution cap 1280 px,
// baseline JPEG, classic xref) so there is ONE compression implementation.
//
// Usage:  node split-person-documents.js [baseDir]
//
// Requires: pdf-lib.
...
function compress(inputPath, outputPath) {
  execFileSync(
    'node',
    [path.join(__dirname, 'compress-pdf.js'), inputPath, outputPath],
    { stdio: 'inherit' }
  );
}
